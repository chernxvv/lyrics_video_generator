...
from tests.fixtures.line_alignment.scenarios import AlignmentScenario, get_scenarios
...
def _scenario_config(scenario: AlignmentScenario, *, use_global_alignment: bool) -> LineAlignmentConfig:
    return replace(scenario.config, use_global_alignment=use_global_alignment)


def _assert_line_alignment_expectation(result, expectation) -> None:
    for idx, expected_status in enumerate(expectation.statuses):
        if expected_status is not None:
            assert result[idx].status == expected_status

    for idx, expected_range in enumerate(expectation.raw_start_ranges):
        if expected_range is None:
            continue
        lower, upper = expected_range
        actual = result[idx].raw_start_seconds
    ...
    for idx, expected_anchor_word in enumerate(expectation.anchor_words):
        if expected_anchor_word is not None:
            assert result[idx].anchor_word == expected_anchor_word

    for idx, expected_anchor_index in enumerate(expectation.anchor_word_indices):
        if expected_anchor_index is not None:
            assert result[idx].anchor_word_index == expected_anchor_index

    for idx, expected_details in enumerate(expectation.detail_equals):
        for key, expected_value in expected_details.items():
            assert result[idx].details[key] == expected_value

    for idx, detail_checks in enumerate(expectation.detail_predicates):
        for key, (operator, expected_value) in detail_checks.items():
            actual = result[idx].details[key]
            if operator == '>':
...
@pytest.mark.parametrize(
    ('scenario'),
    get_scenarios('greedy'),
    ids=lambda scenario: scenario.scenario_id,
)
def test_align_lyric_lines_greedy_fixture_scenarios(scenario: AlignmentScenario) -> None:
    result = align_lyric_lines(
        list(scenario.lyric_lines),
        list(scenario.recognized_words),
        segments=list(scenario.segments) or None,
        config=_scenario_config(scenario, use_global_alignment=False),
    )

    _assert_line_alignment_expectation(result, scenario.expected['greedy'])


@pytest.mark.parametrize(
    ('scenario'),
    get_scenarios('global'),
    ids=lambda scenario: scenario.scenario_id,
)
def test_align_lyric_lines_global_fixture_scenarios(scenario: AlignmentScenario) -> None:
    result = align_lyric_lines(
        list(scenario.lyric_lines),
        list(scenario.recognized_words),
        segments=list(scenario.segments) or None,
        config=_scenario_config(scenario, use_global_alignment=True),
    )

    _assert_line_alignment_expectation(result, scenario.expected['global'])
...
def test_align_lyric_lines_global_keeps_plausible_segment_fallback_after_local_diagnostic(monkeypatch: pytest.MonkeyPatch) -> None:
    scenario = next(item for item in get_scenarios('global') if item.scenario_id == 'fallback_segment')
